...
### Turn saved videos and X articles into knowledge pages

```bash
# 1. Install the only tool required for captioned YouTube videos
brew install yt-dlp

# 2. Sync X bookmarks, process linked videos and enriched X articles, and open the local library
ft app
...
The knowledge library deduplicates YouTube links across bookmarks and imports enriched X Article text already present in the local bookmark cache. For videos, it prefers creator or automatic captions and falls back to local `whisper.cpp` transcription when captions are unavailable. Both source types receive navigation, citation-grounded summaries, library search, cited item chat, related-page discovery, and local notes. The page appears progressively while background work continues.
...
| `ft app` | Run the private local knowledge library, sync bookmarks, and process linked YouTube videos and enriched X articles |
